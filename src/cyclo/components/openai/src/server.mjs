...
export async function createOpenAIComponentServer({
  services,
  shutdownSignal,
  ...
  const provider = bindings.requires.get("provider");
  if (bindings.provides.size !== 1
      || bindings.provides.get(Component.typeName) !== Component
      || bindings.requires.size !== 1
      || provider?.typeName !== Provider.typeName) {
    throw new TypeError(
      "openai must provide Component and require exactly one provider Provider",
    );
  }
