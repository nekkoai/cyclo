...
export async function createPoolerServer({
  services,
  shutdownSignal,
  componentConf = defaultComponentConf,
} = {}) {
  if (!services?.component || !services?.provider) {
    throw new TypeError("component and provider services are required");
  }
  ...
  if (bindings.requires.size !== 1 || upstream?.typeName !== Provider.typeName) {
    throw new TypeError("pooler must require exactly one upstream Provider");
  }
