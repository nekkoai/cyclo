...
def test_release_manifest_for_built_distributions(
    ...
    sbom = json.loads((tmp_path / "cyclo-agent-0.2.5.spdx.json").read_text())
    ...
    assert any(
        reference["referenceLocator"] == "pkg:pypi/cyclo-agent@0.2.5"
...
def test_release_manifest_cross_checks_sdist_metadata(
    ...
    with tarfile.open(sdist, mode="w:gz") as archive:
        member = tarfile.TarInfo("cyclo_agent-0.2.5/PKG-INFO")
